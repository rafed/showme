...
def get_citation(citation):
	...
	candidate_citation_regex_result=re.findall(r'(\[.*?\])',citation)
	candidate_citations=[]
	valid_citations={}
	searched_citations=[]
	...
	for candidate_citation in candidate_citations:
		...
		if(transformed_candidate_citation.isdigit()):
	...
	for key,value in valid_citations.iteritems():
		...
		for cite in regex_extract_searched.split(numeric_key):
			#print "key "+key
			snippets.append((str(cite),text))
my_file = os.path.join("cube.pdf")
